...
public class CustomUserRepository {
    ...
    public CustomUserRepository(UserRepository userRepository) {
        this.userRepository = userRepository;
    }

    /*public List<User> getLowRangeProducts(List<Category> categories) {
        return userRepository.findAll(where(belongsToCategory(categories)).and(pricesAreBetween(0D, PREMIUM_PRICE)));
    }
    ...
    public List<User> getQueryResult(List<Filter> filters){
        if(filters.size()>0) {
    ...
    private Specification<User> userNameLike(String userName){
        return (root, query, criteriaBuilder) -> criteriaBuilder.like(root.get(User_.USERNAME), "%"+userName+"%");
    }
